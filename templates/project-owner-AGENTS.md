...
- Use `flocky_dispatch` for a direct durable task to a configured stream; it selects that stream’s configured transport. For a simple quoted message, pass only the quoted text as the task (for example, `send "hi" to stream-a` becomes task `hi`), never an instruction to resend it. Use `flocky_delegate` for repository work when durable versus transient execution should be selected intentionally.
